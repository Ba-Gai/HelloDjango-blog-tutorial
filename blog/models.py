...
class Category(models.Model):
    ...
    class Meta:
        ...
        # 用来表示多篇文章时的复数显示形式。英语中，如果有多篇文章，就会显示为 Posts，表示复数，
        # 中文没有复数表现形式，所以定义为和 verbose_name一样。
...
class Post(models.Model):
    ...
    '''
    需求：在Django博客开发中，当点击一篇文章的继续阅读,展开整篇文章。
    我们要获取这样的url:......./post/3
    现在利用reverse()方法获取url:
    第一个参数：" blog:detal ":  blog 应用下的 name = detail 的函数。
    detail 对应的正则表达式 ：post /(?P<pk>[0-9]+)/
    第二个参数：self.pk会替代正则表达式里面的pk
    然后 reverse 函数区解析视图函数对应的URL。
    '''
